...
async def create_benchmark(
    ...
    # from the database using its own connection so that large embedding
    # vectors (100+ chunks x 1536 floats) are never serialised across a
    # thread boundary as a task argument, which caused silent hangs on
    # Render's free tier (legacy hosting; current platform is Railway).
    pool = await get_pool()
    run_ids: list[str] = []

    logger.info(
        "benchmark dispatch: n_strategies=%d corpus_hash=%s",
        len(body.strategies),
        body.corpus_hash,
    )
    ...
    for item in body.strategies:
        ...
        logger.info("scheduled run_id=%s strategy=%s", run_id, item.strategy)
